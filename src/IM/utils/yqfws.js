...
export class ServingClient {
    ...
    static invoke = (svMethod, svParam, success, fail, svFormat = 'JSON', url = 'https://api2.yqfws.com/') => {
        ...
        if(Chat.obj.Source =='抢单'){
            url = url + `?app_key=100041&method=${svMethod}`;
            secret="4625c178408fbb42";

        }else if(Chat.obj.Source =='我去过'){
            url = url + `?app_key=100038&method=${svMethod}`;
            secret="2e4578df4313a047";
        }else{
        ...
        let body = ServingClient.encodingPostData(svParam, secret);
...
export class RestAPI {
    ...
    static invoke = (svMethod, svParam, success, fail, svFormat = 'JSON', url = 'https://www2.yqfws.com/rest') => {
        ...
        // var appkey,appSecret;//全部调用老接口的我去过的key和screct
            ...
            if (res.ok) {
                if (svFormat.toUpperCase() == 'JSON') {
                    res.json().then((json) => {
                      ...
                      //  if (BaseComponent.getLocale().lang == "EN") BaseComponent.replaceData(json);
                        success(json);
