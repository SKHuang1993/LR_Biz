...
class InputItem extends React.Component {
    ...
    render() {
        const { prefixCls, prefixListCls, type, value, defaultValue, name, editable, disabled, style, clear, children, error, className, extra, labelNumber, maxLength, } = this.props;
        ...
        <div className={`${prefixCls}-control`}>
          <input ref="input" {...patternProps} {...otherProps} {...valueProps} type={inputType} maxLength={maxLength} name={name} placeholder={placeholder} onChange={this.onInputChange} onBlur={this.onInputBlur} onFocus={this.onInputFocus} readOnly={!editable} disabled={disabled}/>
        </div>
        {clear && editable && !disabled && (value && value.length > 0) ?
            <div className={`${prefixCls}-clear`} onTouchStart={() => { this.clearInput(); }}/>
            : null}
