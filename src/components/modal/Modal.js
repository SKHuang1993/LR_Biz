...
class AntmModal extends React.Component {
    ...
    render() {
        const { title, closable, footer, children, style, animateAppear, transparent, visible, onClose, bodyStyle, onAnimationEnd, styles, } = this.props;
        ...
        const footerDom = footer && footer.length ? (<View style={[btnGroupStyle, styles.footer]} onLayout={this.onFooterLayout}>
          {footer.map((button, i) => {
            let buttonStyle = {};
            if (button.style) {
            ...
            return (<TouchableHighlight key={i} style={horizontalFlex} underlayColor="#ddd" onPress={() => {
                if (button.onPress) {
                    button.onPress();
                }
                if (onClose) {
        ...
        if (transparent) {
            ...
            return (<RCModal onClose={this.onMaskClose} animationType={animType} wrapStyle={transparent ? styles.container : undefined} style={[styles.innerContainer, style]} visible={visible} onAnimationEnd={onAnimationEnd} animateAppear={animateAppear}>
        ...
        return (<Modal visible={visible} animationType={animType} onRequestClose={this.onMaskClose}>
        <View style={style}>
          {children}
        </View>
      </Modal>);
