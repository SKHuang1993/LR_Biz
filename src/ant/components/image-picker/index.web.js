...
export default class ImagePicker extends React.Component {
    ...
    render() {
        ...
        files.forEach((image, index) => {
            imgItemList.push(<div key={index} className={`${prefixCls}-item`} style={itemStyle}>
          <div className={`${prefixCls}-item-remove`} onClick={() => { this.removeImage(index); }}/>
          <div className={`${prefixCls}-item-content`} onClick={() => { this.onImageClick(index); }} style={{ backgroundImage: `url(${image.url})` }}/>
        </div>);
        });
        ...
        <div className={`${prefixCls}-list`}>
          <WingBlank size="md">
            <Flex wrap="wrap">
              {imgItemList}
              {selectable && <Touchable activeClassName={`${prefixCls}-upload-btn-active`}><div className={`${prefixCls}-item ${prefixCls}-upload-btn`} style={itemStyle} onClick={() => {
            if (onAddImageClick) {
                onAddImageClick();
            }
        }}>
                {!onAddImageClick ? <input style={itemStyle} ref="fileSelectorInput" type="file" accept="image/jpg,image/jpeg,image/png,image/gif" onChange={() => { this.onFileChange(); }}/> : null}
              </div></Touchable>}
            </Flex>
          </WingBlank>
        </div>
