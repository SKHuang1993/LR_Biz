...
function createActionSheet(flag, config, callback) {
    ...
    switch (flag) {
        ...
        <div className={`${prefixCls}-button-list`}>
          {options.map((item, index) => {
                ...
                let bItem = (<Touchable key={index} activeClassName={`${prefixCls}-button-list-item-active`}>
                <div {...itemProps}>{item}</div>
              </Touchable>);
                if (cancelButtonIndex === index || destructiveButtonIndex === index) {
                    bItem = (<Touchable key={index} activeClassName={`${prefixCls}-button-list-item-active`}>
                  <div {...itemProps}>
                    {item}
        ...
        case SHARE:
            ...
            const createList = (item, index, rowIndex = 0) => (<div className={`${prefixCls}-share-list-item`} key={index} onClick={() => cb(index, rowIndex)}>
    ...
    ReactDOM.render(<Dialog visible title="" footer="" prefixCls={prefixCls} className={rootCls} transitionName={transitionName || `am-slide-up`} maskTransitionName={maskTransitionName || `am-fade`} onClose={close} maskClosable={maskClosable} wrapProps={props.wrapProps || {}}>{children}</Dialog>, div);
    return {
        close,
    };
}
