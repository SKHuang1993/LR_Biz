...
export default class Grid extends React.Component {
    ...
    render() {
            ...
            for (let pageIndex = 0; pageIndex < pageCount; pageIndex++) {
                ...
                for (let ii = 0; ii < carouselMaxRow; ii++) {
                    ...
                    if (rowIndex < rowCount) {
                        ...
                        pageRows.push(<Flex key={rowIndex} style={[styles.grayBorderBox, { borderBottomWidth: hasLine ? 1 : 0 }]}>
              {res}
            </Flex>);
                    }
                }
        ...
        return isCarousel && pageCount > 1 ? (<Carousel infinite={false} dots>{pagesArr}</Carousel>) : <Flex direction="column">{rowsArr}</Flex>;
    }
}
Grid.defaultProps = {
    data: [],
