    ...
    render() {
      ...
      <List>
          ...
          {Array.apply(null, Array(20)).map(function (_, i) { return i; }).map((_i, index) => {
            ...
            return (<List.Item key={index} thumb="https://zos.alipayobjects.com/rmsportal/eOZidTabPoEbPeU.png"><Text>分类 - {index}</Text></List.Item>);
        })}
        ...
        return (<Drawer sidebar={sidebar} {...drawerProps} ref={d => {
            this.drawer = d;
...
export class DrawerMain extends React.Component {
    ...
    render() {
        return (<View style={{ flex: 1, marginTop: 114, padding: 8 }}>
