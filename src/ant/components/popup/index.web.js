...
function create(instanceId, config, content, afterClose = (_x) => { }) {
    const props = assign({}, {
        prefixCls: 'am-popup',
    ...
    ReactDOM.render(<Dialog prefixCls={prefixCls} visible title="" footer="" className={`${prefixCls}-${animationType}`} transitionName={transitionName || transName} maskTransitionName={maskTransitionName || 'am-fade'} maskClosable={maskClosable} wrapProps={props.wrapProps || {}} maskProps={props.maskProps || maskProps}>{content}</Dialog>, div);
    return {
        instanceId,
        close,
    };
