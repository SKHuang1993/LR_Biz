...
export default React.createClass({
    ...
    render() {
        ...
        return (<ListView dataSource={this.state.dataSource} renderHeader={() => <Text style={[{ padding: 30 }]}>列表头</Text>} renderFooter={() => <Text style={[{ padding: 30, textAlign: 'center' }]}>
          {this.state.isLoading ? '加载中...' : '加载完毕'}
        </Text>} renderSectionHeader={(sectionData) => <Text style={[{
                padding: 10, color: 'blue',
                backgroundColor: 'rgba(150, 150, 150, 0.6)',
            }]}>
            {`任务 ${sectionData.split(' ')[1]}`}
          </Text>} renderRow={row} renderSeparator={separator} pageSize={4} onEndReached={this.onEndReached} onEndReachedThreshold={10} onChangeVisibleRows={(_visibleRows, _changedRows) => {
